...
describe("POST /api/products", () => {
    ...
    it('should validate that the price is greater that 0',  async () => {
        ...
        expect(response.status).toBe(400)
        expect(response.body).toHaveProperty('erros')
        expect(response.body.erros).toHaveLength(2)
        
        expect(response.status).not.toBe(404)
        expect(response.body.erros).not.toHaveLength(4)
    })
...
describe("GET /api/products/:id", () => {
    ...
    it("Should check a valid ID in the URL", async () => {
        ...
        expect(response.body).toHaveProperty('erros')
        expect(response.body.erros).toHaveLength(1)
        expect(response.body.erros[0].msg).toBe('ID no valido')
...
describe("PUT /api/products/:id", () => {
    ...
    it("should display validation error message when updating a product", async () => {
        ...
        expect(response.status).toBe(400)
        expect(response.body).toHaveProperty('erros')
        expect(response.body.erros).toBeTruthy()
        expect(response.body.erros).toHaveLength(5)
    ...
    it("should validate that the price is greater that 0", async () => {
        ...
        expect(response.body.erros).toBeTruthy()
        expect(response.body.erros).toHaveLength(1)
        expect(response.body.erros[0].msg).toBe('Precio no valido')
    ...
    it("should return 404 response for a non-existend product", async () => {
        const productId = 2000
        const response = await request(server)
